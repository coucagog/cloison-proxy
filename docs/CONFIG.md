...
| `CLOISON_REALISTIC_FAKE` *(dette 3)* | edge | non | `0` | **opt-in** : la politique remplace PERSON / Gazetteer(nom_sn) / PhoneSn / Email par un **faux réaliste déterministe dans la session** (graine = corps du jeton) au lieu des sentinelles — pour les modèles qui nettoient les ⟦…⟧ (constat deepseek, 03/09/2026). **IRRÉVERSIBLE** (la valeur d'origine ne revient jamais au client) ; types sans générateur → sentinelle (sûr). Ne jamais mélanger avec des sentinelles sans vérifier la restauration complète avant émission |
...
| **`X-Cloison-Tenant`** *(C, multi-tenant)* | edge (header) | non | tenant par défaut (`CLOISON_TENANT_ID`) | **header de ROUTAGE** (charte §7.2, non secret) : sélectionne le tenant auprès duquel le jeton `mn_` est vérifié par hash. Sans header → tenant par défaut. Un jeton n'est accepté que dans le tenant où il a été émis (fail-closed) ; les reçus d'audit sont tagués au tenant de la requête et l'ingest est groupé par tenant. Header invalide (format) → repli tenant par défaut |
