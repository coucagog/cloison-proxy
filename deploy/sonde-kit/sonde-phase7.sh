#!/usr/bin/env bash
# =============================================================================
# sonde-phase7.sh — edge en MASQUAGE ACTIF (CLOISON_AUDIT_MODE=0) + vault/NER
# (le crash "hash audit policy" était le combo N0+AUDIT_MODE=1 ; avec audit
#  off, le mode N0 + NER embarqué devrait booter — sinon repli sans vault.)
# =============================================================================
set -uo pipefail
...
start_edge(){
  ...
  sleep 4
  docker inspect "$EDGE" --format '{{.State.Status}}'
}

ST="$(start_edge 0)"
echo "EDGE state=$ST (tentative avec vault+NER)"
if [ "$ST" != "running" ]; then
  echo "--- logs boot (vault+NER) ---"; docker logs "$EDGE" --tail 12 2>&1 | tail -12
  echo "REPLI : sans vault/NER"
  ST="$(start_edge 1)"
  echo "EDGE state=$ST (repli sans vault)"
fi
if [ "$ST" != "running" ]; then echo "--- logs boot (repli) ---"; docker logs "$EDGE" --tail 12 2>&1 | tail -12; exit 1; fi
docker network disconnect none "$EDGE" >/dev/null 2>&1 || true
docker network connect "$NET" "$EDGE" || { echo "FAIL raccord edge"; exit 1; }
echo "EDGE RACCORDE (MASQUAGE ACTIF)"
sleep 2
...
# --- Preuves --------------------------------------------------------------------------
echo "=== CE QUE LE MOCK A RECU (anti-pass-through : attendu des sentinelles) ==="
docker logs "$MOCK" --tail 6 2>&1 | sed -E 's@(mn_)?[a-f0-9]{16,}@<CAVIARDE>@g' || true
echo "=== LOGS EDGE (tail 15) ==="
docker logs "$EDGE" --tail 15 2>&1 | sed -E 's@(mn_)?[a-f0-9]{16,}@<CAVIARDE>@g' || true

echo "=== PHASE7 TERMINEE — depouillement : sudo bash /tmp/sonde-cleanup.sh ==="
